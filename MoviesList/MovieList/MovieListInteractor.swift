...
protocol MovieBusinessLogic {
    func fetchMovies(request: MovieListModels.FetchMovies.Request)
...
//        func fetchMovies(request: MovieListModels.FetchMovies.Request) {
//            worker.fetchMovies(apiKey: apiKey, currentPage: currentPage) { [weak self] movies in
//                guard let self = self else { return }
//                self.movies = movies
//
//                print("Selected Moviesss: \(String(describing: self.selectedMovie))")
...
//        }
    ...
    func fetchMovies(request: MovieListModels.FetchMovies.Request) {
            worker.fetchMovies(apiKey: apiKey, currentPage: currentPage) { [weak self] movies in
                ...
                // Assuming you want to set the first movie as the selected movie, adjust this logic as needed
